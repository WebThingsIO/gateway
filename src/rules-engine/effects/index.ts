...
type EffectClass = { new(addonManager: AddonManager, desc: any): any; };
 ...
 * @return {Effect}
 */
export function fromDescription(addonManager: AddonManager, desc: EffectDescription): Effect {
  const EffectClass = effects[desc.type];
  if (!EffectClass) {
    throw new Error(`Unsupported or invalid effect type:${desc.type}`);
  }
  return new EffectClass(addonManager, desc);
}
