...
it('edits a user', async () => {
  ...
  await editUser(server, jwt, Object.assign({}, TEST_USER_UPDATE_1, { id: info.id }));
...
it('edits a user, including password', async () => {
  const jwt = await createUser(server, TEST_USER);
  let info = (await userInfo(server, jwt))!;
  await editUser(server, jwt, Object.assign({}, TEST_USER_UPDATE_2, { id: info.id }));
  info = (await userInfo(server, jwt))!;
  ...
  await loginUser(
    server,
    Object.assign({}, TEST_USER_UPDATE_2, { password: TEST_USER_UPDATE_2.newPassword })
  );
});
...
it('logs in successfully with MFA token', async () => {
  ...
  const jwt = await loginUser(server, TEST_USER);
  const info = (await userInfo(server, jwt))!;
  const params = await enableMfa(server, jwt, Object.assign({}, TEST_USER, { id: info.id }));
  await logoutUser(server, jwt);
...
it('enables, disables, and re-enables MFA, but gets new secret', async () => {
  ...
  let jwt = await loginUser(server, TEST_USER);
  const info = (await userInfo(server, jwt))!;
  const params1 = await enableMfa(server, jwt, Object.assign({}, TEST_USER, { id: info.id }));
  await logoutUser(server, jwt);
  ...
  const params2 = await enableMfa(server, jwt, Object.assign({}, TEST_USER, { id: info.id }));
