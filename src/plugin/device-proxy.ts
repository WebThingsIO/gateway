...
import {Device, Action as AddonAction, Event as AddonEvent, Property} from 'gateway-addon';
...
export default class DeviceProxy extends Device {
  ...
  notifyPropertyChanged(property: Property<PropertyValue>): void {
    this.getAdapter().getManager().emit(MessageType.PROPERTY_CHANGED, property);
  }
  ...
  actionNotify(action: AddonAction): void {
    ...
    this.getAdapter().getManager().emit(MessageType.ACTION_STATUS, action);
  }

  eventNotify(event: AddonEvent): void {
    Events.add(new Event(
      event.asDict().name, event.asDict().data, this.getId(), event.asDict().timestamp));
    this.getAdapter().getManager().emit(MessageType.EVENT, event);
  }

  connectedNotify(connected: boolean): void {
    this.getAdapter().getManager().emit(MessageType.CONNECTED, {device: this, connected});
  }
}
