...
export default function InternalLogsController(addonManager: AddonManager): express.Router {
 ...
 */
  ...
  router.ws('/', (websocket: WebSocket) => {
    ...
    addonManager.getPluginServer().on('log', onLog);

    const cleanup = () => {
      addonManager.getPluginServer().removeListener('log', onLog);
