...
interface Log {
  severity: Constants.LogSeverity;
  message: string;
  url?: string;
}
  ...
  controller.get('/', async (request, response) => {
    ...
    for (const name of files) {
      if (fs.lstatSync(path.join(UserProfile.logDir, name)).isFile()) {
        content +=
          `${'<li>' +
          `<a href="${Constants.INTERNAL_LOGS_PATH}/files/${encodeURIComponent(name)}?jwt=${
            jwt}">`}${Utils.escapeHtml(name)}</a>` +
          `</li>`;
      }
    }
