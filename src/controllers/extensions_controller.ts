...
export default function ExtensionsController(addonManager: AddonManager): express.Router {
  ...
  router.all(
    ...
    async (request, response) => {
      ...
      try {
        ...
        const contentType = rsp.getContentType();

        if (contentType && rsp.getContent() !== null &&
   ...
   */
  router.get('/:extensionId/*', (request, response) => {
    const extensionId = request.params.extensionId;
