...
export default function ThingsController(addonManager: AddonManager): express.Router {
 ...
 */
  router.ws('/:thingId/', websocketHandler);
    ...
    if ((request as any).jwt.payload.role !== Constants.USER_TOKEN) {
      if (!(request as any).jwt.payload.scope) {
        ...
        scope.split(':')[0] === Constants.THINGS_PATH) {
          addonManager
            .getThingsCollection()
            .getThingDescriptions(request.get('Host'), request.secure).then((things: any[]) => {
        ...
        // Get hrefs of things in scope
          ...
          addonManager
            .getThingsCollection()
            .getListThingDescriptions(hrefs, request.get('Host'), request.secure)
            .then((things: any[]) => {
              response.status(200).json(things);
            });
        }
 ...
 */
  router.post('/', async (request, response) => {
    ...
    // If the thing already exists, bail out.
      await addonManager.getThingsCollection().getThing(id);
    ...
    try {
      const thing = await addonManager.getThingsCollection().createThing(id, description);
 ...
 */
  router.get('/:thingId', (request, response) => {
    ...
    addonManager
      .getThingsCollection()
      .getThingDescription(id, request.get('Host'), request.secure).then((thing: any) => {
 ...
 */
  router.get('/:thingId/properties', async (request, response) => {
    const thingId = request.params.thingId;

    let thing;
    try {
      thing = await addonManager.getThingsCollection().getThing(thingId);
    } catch (e) {
 ...
 */
  router.get(
    ...
    async (request, response) => {
      ...
      try {
        const value = await addonManager
          .getThingsCollection()
          .getThingProperty(thingId, propertyName);
        const result: any = {};
 ...
 */
  router.put(
    ...
    async (request, response) => {
      ...
      try {
        const updatedValue = await addonManager
          .getThingsCollection()
          .setThingProperty(thingId, propertyName,
                            value);
        const result = {
 ...
 */
  router.use(`/:thingId${Constants.ACTIONS_PATH}`, ActionsController(addonManager));

  /**
 * Use an EventsController to handle each thing's events.
 */
  router.use(`/:thingId${Constants.EVENTS_PATH}`, EventsController(addonManager));

  /**
 * Modify a Thing's floorplan position or layout index.
 */
  router.patch('/:thingId', async (request, response) => {
    const thingId = request.params.thingId;
    if (!request.body) {
 ...
 * Modify a Thing.
 */
  router.put('/:thingId', async (request, response) => {
    const thingId = request.params.thingId;
 ...
 * Remove a Thing.
 */
  router.delete('/:thingId', (request, response) => {
    const thingId = request.params.thingId;

    const _finally = () => {
      addonManager.getThingsCollection().removeThing(thingId).then(() => {
  ...
  // possibility that the WebSocket have been closed.
    ...
    if (typeof thingId !== 'undefined') {
      addonManager.getThingsCollection().getThing(thingId).then((thing: any) => {
        addThing(thing);
      }).catch(() => {
      ...
      addonManager.getThingsCollection().getThings().then((things: Map<string, Thing>) => {
        things.forEach(addThing);
      });
      addonManager.getThingsCollection().on(Constants.THING_ADDED, onThingAdded);
    }

    addonManager.on(Constants.PROPERTY_CHANGED, onPropertyChanged);
    addonManager
      .getActionsCollection().on(Constants.ACTION_STATUS, onActionStatus);
    ...
    const cleanup = () => {
      addonManager.getThingsCollection().removeListener(Constants.THING_ADDED, onThingAdded);
      addonManager.removeListener(Constants.PROPERTY_CHANGED, onPropertyChanged);
      addonManager
        .getActionsCollection().removeListener(Constants.ACTION_STATUS, onActionStatus);
      ...
      switch (request.messageType) {
        ...
        case Constants.REQUEST_ACTION: {
          for (const actionName in request.data) {
            ...
            addonManager.getThingsCollection().getThing(id).then((thing: any) => {
              const actionId = addonManager
                .getActionsCollection()
                .generateId();
              const action = new Action(actionId, actionName, actionParams, thing);
              return addonManager
                .getActionsCollection().add(action).then(() => {
