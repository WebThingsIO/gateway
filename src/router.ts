...
import RuleEngine from './rules-engine/index';
...
class Router {
  ...
  configure(app: express.Application) {
    ...
    app.use(API_PREFIX + Constants.RULES_PATH, nocache, auth, RuleEngine);
  ...
  addProxyServer(thingId: string, server: string) {
    this.proxyStore?.addProxyServer(thingId, server);
  }

  removeProxyServer(thingId: string) {
