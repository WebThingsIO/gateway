...
add-thing-add-adapters-hint = Ничего не найдено. Попробуйте
add-thing-add-adapters-hint-anchor = добавить дополнения.
...
local-token-jwt = Веб-токен JSON
local-token-use-it = Используйте его для общения со шлюзом, с
